...
function extractArticleBadge(article?: string | null) {
  ...
  if (String(article).toLowerCase().includes("article")) return String(article).replace(/MDR.*?â€“\s*/i, "").trim();
  return String(article).trim();
}

function formatCriticality(level?: string | null) {
  const v = (level || "").toLowerCase().trim();
  if (!v) return { label: "CriticitÃ© n/a", variant: "outline" as const };
  if (v === "critical" || v === "very_high" || v === "high")
    return { label: "CriticitÃ© Ã©levÃ©e", variant: "destructive" as const };
  if (v === "medium") return { label: "CriticitÃ© moyenne", variant: "secondary" as const };
  if (v === "low") return { label: "CriticitÃ© faible", variant: "outline" as const };
  return { label: `CriticitÃ© ${level}`, variant: "outline" as const };
}
...
function aiInsightsForQuestion(
  ...
  const base = [
    `Ce que l'ON cherche : preuve d'exÃ©cution rÃ©elle sur ${extractArticleBadge(question.article) || "l'exigence visÃ©e"}.`,
    "OÃ¹ creuser : cohÃ©rence entre procÃ©dure, enregistrements et pratiques terrain.",
    "Qui interviewer : responsable process, PRRC et opÃ©rateur de premiÃ¨re ligne.",
  ];

  if (valueNow === "partial") {
    return [
      ...base,
      "Risque probable : NC mineure Ã©voluant en majeure si le pattern se rÃ©pÃ¨te.",
      "Impact certification : action corrective exigÃ©e avec dÃ©lai contraint.",
      "Question complÃ©mentaire : montrer une preuve terrain datÃ©e des 90 derniers jours.",
    ];
  }
  ...
  if (valueNow === "non_compliant") {
    return [
      ...
      "Impact patient : signal faible de dÃ©tection tardive possible.",
      "PrioritÃ© : vÃ©rifier cohÃ©rence PMS vs CER et dÃ©clenchement CAPA.",
    ];
  }
  ...
  if (expertMode || inspectorMode) {
    return [
      ...base,
      "Angle mort classique ON : procÃ©dure valide mais non appliquÃ©e en routine.",
      "PiÃ¨ge rÃ©glementaire : KPI sans seuil d'alerte opÃ©rationnel.",
    ];
  }
...
function formatRiskText(risk: any): string {
  ...
  // âœ… FIX: if risk is JSON string like '["..."]', extract first string for display
...
// âš ï¸ IMPORTANT: do NOT use hooks (useMemo) for this below conditional returns.
...
export default function GenericAuditQuestionnaire() {
  ...
  const {
    ...
    error: contextError,
  } = trpc.audit.getAuditContext.useQuery({ auditId: (auditId ?? 0) as number }, { enabled });

  // âœ… We reuse the same processes source as MDR (single truth)
  ...
  } = trpc.audit.getQuestionsForAudit.useQuery({ auditId: (auditId ?? 0) as number }, { enabled });

  const questions: Question[] = useMemo(() => {
    const q = (questionsPayload as any)?.questions;
    const all = Array.isArray(q) ? (q as Question[]) : [];

    // âœ… Safety-net: if backend returns unfiltered questions (processIds empty or mismatch),
  ...
  } = trpc.audit.getResponses.useQuery({ auditId: (auditId ?? 0) as number }, { enabled });
  ...
  const completeAuditMutation = trpc.audit.completeAudit.useMutation();
  ...
  // âœ… Non-blocking warning: responses load may fail, but we can still run the audit
  ...
  // âœ… Ensure the risk banner ALWAYS follows the current question.
  ...
  const handleSetCompliance = (value: ResponseValue) => {
    ...
    // âœ… FIX: link response to the question's processId first
    const qProcessId =
  ...
  const handleSaveCurrent = async () => {
    ...
    const mergedRow: ResponseRow = {
      ...
      // âœ… FIX: preserve per-question processId
      processId:
    ...
    if (!hasAnyValue) {
      setSaveMessage("Rien Ã  enregistrer");
      return true;
    }
    ...
    try {
      ...
      setSaveMessage("EnregistrÃ© âœ…");

      // garde la ligne localement pour que progression et score restent cohÃ©rents immÃ©diatement
      setLocalDrafts((prev) => ({
        ...prev,
      ...
      console.error(e);
      setSaveMessage("Erreur dâ€™enregistrement");
      return false;
    } finally {
      setSaving(false);
    }
  ...
  // âœ… FIX: ISO wizard back route (was /mdr)
  const goBackToWizard = () => setLocation("/audit/new");
          ...
          <CardContent className="p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground">ID dâ€™audit manquant ou incorrect.</div>
              </div>
            </div>
      ...
      <div className="p-6">
        <Card>
          <CardContent className="p-10 flex items-center justify-center gap-3">
            ...
            <div>Chargement du questionnaireâ€¦</div>
          </CardContent>
        </Card>
      </div>
    );
          ...
          <CardContent className="p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground">
                  Aucune question nâ€™a Ã©tÃ© trouvÃ©e pour cet audit (filtrage rÃ´le/process/rÃ©fÃ©rentiel).
                </div>
              </div>
            </div>
  ...
  const aiInsights = aiEnabled
    ...
    : ["Copilot dÃ©sactivÃ©. Activez IA ON pour obtenir des suggestions contextuelles."];
          ...
          <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-3">
            <div>
              <div className="text-base md:text-lg font-semibold text-slate-900">
                {(auditContext as any)?.auditName || `Audit #${auditId}`}
              </div>
              <div className="text-sm text-muted-foreground">
                Progression {progressPct}% â€¢ Score conformitÃ© live {complianceScore}%
              </div>
            </div>

            {responsesWarning && (
              <div className="mt-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900">
                âš ï¸ Certaines rÃ©ponses existantes nâ€™ont pas pu Ãªtre chargÃ©es (vous pouvez quand mÃªme continuer lâ€™audit).
          ...
          <CardContent className="p-5 md:p-6 space-y-6">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="text-[11px] uppercase tracking-[0.08em] font-semibold text-rose-700">
                      Risque Ã  justifier en cas de non-conformitÃ©
                    </div>
                  ...
                  <Badge className="bg-rose-100 text-rose-700 border border-rose-200 hover:bg-rose-100">
                    GravitÃ©: {crit.label.replace("CriticitÃ© ", "")}
                  </Badge>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                placeholder="DÃ©crivez les constats terrain, la cohÃ©rence documentaire, les preuves vÃ©rifiÃ©es, les Ã©carts observÃ©s et le lien direct avec le risque rÃ©glementaire/patient."
                className="min-h-[140px]"
              />
            </div>

            <div className="space-y-2">
              <div className="text-sm font-medium">Statut de conformitÃ©</div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Ex: pÃ©rimÃ¨tre limitÃ©, justification d'applicabilitÃ©, dÃ©cision de traitement du risque..."
                className="h-11"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              {evidenceNames.length > 0 ? (
                ...
                <div className="text-xs text-muted-foreground">Aucun document sÃ©lectionnÃ©.</div>
              )}
            </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-muted-foreground">
              <div className="flex items-center gap-2">
                ...
                {saveMessage ? <span className="font-medium">{saveMessage}</span> : <span>Brouillon local actif jusquâ€™Ã  enregistrement.</span>}
              </div>
          ...
          <CardContent className="p-4 space-y-4">
            ...
            {activeTab === "context" && (
              <div className="space-y-3 text-sm">
                ...
                <div>
                  <div className="text-muted-foreground">RÃ´le Ã©conomique</div>
              ...
              <div className="space-y-3">
                ...
                {inspectorMode ? (
                  <div className="rounded-md border border-amber-300 bg-amber-50 p-3 text-sm">
                    ...
                    <div>"Montrez la preuve terrain la plus rÃ©cente qui contredit le moins votre procÃ©dure."</div>
                  </div>
                ) : null}

                {showCoherenceAlert ? (
                  <div className="rounded-md border border-red-300 bg-red-50 p-3 text-sm text-red-800">
                    Attention : signaux faibles dÃ©tectÃ©s. VÃ©rifier cohÃ©rence PMS / CER / CAPA avant clÃ´ture.
                  </div>
                ) : null}
                ...
                <Button
                  ...
                  variant="outline"
                  className="w-full"
                  onClick={() => setSaveMessage("IA : recommandations gÃ©nÃ©rÃ©es localement (mode soft)")}
                >
                  <Sparkles className="mr-2 h-4 w-4" />
                  RafraÃ®chir suggestions
                </Button>
              </div>
            )}
            ...
            {activeTab === "evidence" && (
              <div className="space-y-3 text-sm">
                ...
                <div className="whitespace-pre-wrap text-muted-foreground">
                  {currentQuestion?.expectedEvidence || "Aucune preuve attendue spÃ©cifiÃ©e pour cette question."}
                </div>
                ...
                {currentQuestion?.interviewFunctions && currentQuestion.interviewFunctions.length > 0 ? (
                  <>
                    <Separator />
                    <div className="space-y-2">
                      <div className="font-medium">Fonctions Ã  interviewer</div>
      ...
      <Card className="shadow-sm border-slate-200 bg-white">
        <CardContent className="p-4">
          <div className="flex items-center justify-between gap-2">
            ...
            <Button type="button" variant="outline" size="sm" onClick={() => setSaveMessage("Mode impression rapport : bientÃ´t") }>
