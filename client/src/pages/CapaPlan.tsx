        ...
        <div className="rounded-lg border border-violet-200 bg-violet-50/50 p-4">
          ...
          {aiAnalysis && (
            <div className="mt-4 space-y-4">
              ...
              {Array.isArray(aiAnalysis.sequenceRecommandee) && aiAnalysis.sequenceRecommandee.length > 0 && <div className="space-y-2 rounded border bg-white p-3"><div className="text-sm font-medium">Chronologie recommandée</div>{aiAnalysis.sequenceRecommandee.map((step:any,index:number)=>{const actionProposee=aiAnalysis.actionsCorrectivesProposees.find((item:any)=>item.id===step.actionId);return <div key={`${step.actionId}-${index}`} className="grid grid-cols-[5rem_1fr_5rem] items-center gap-2 text-sm"><Badge variant="outline">{step.debut}</Badge><div className="h-2 rounded bg-primary/20"><div className="h-2 rounded bg-primary" /></div><Badge variant="outline">{step.fin}</Badge><span className="col-span-3 text-xs text-muted-foreground">{actionProposee?.titre ?? `Action ${step.actionId}`}</span></div>})}<p className="text-xs text-muted-foreground">Les échéances individuelles seront enregistrées sur chaque action CAPA. Elles restent modifiables après création.</p></div>}
